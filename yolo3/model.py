...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    # YOLOv3 的先验框anchor box一共有3组共9个,特征图越小，感受野越大，对大目标越敏感，选用大的anchor box；特征图越大，感受野越小，对小目标越敏感，选用小的anchor box
    # num_anchors = 3(13x13特征图对应：【（116*90），（156*198），（373*326）】26x26特征图对应：【（30*61），（62*45），（59*119）】52x52特征图对应：【（10*13），（16*30），（33*23）】)
    # num_classes是检测类别的数量（比如：person、car、sports ball等，在YOLOv3提供的训练好的模型中，num_classes = 80）。
    x, y1 = make_last_layers(darknet.output, 512, num_anchors*(num_classes+5))  #x.shape=[?,13,13,512],y1.shape = [?,13,13,num_anchors*(num_class+5)]
    #y1：输入是darnknet.output，shape是[?,13,13,1024]，作为第1个make_last_layers()的输入，输出x的shape是[?, 13, 13, 512]，输出y1的shape是[?, 13, 13, num_anchors*(num_classes+5) ]

    # y2: 输入是（x,y1)输出的x，shape是[?, 13, 13, 512]。
    # 先执行一次有256个滤波器（所以，输出是256个通道）的1x1卷积操作DarknetConv2D_BN_Leaky()；
    # 然后做一次上采样操作UpSampling2D()，将输入为13x13的结构，转换为26x26的结构。x.shape = [?,26,26,256]
    # 将x与darknet的第152层拼接Concatenate（拼接后张量的shape是[?, 26, 26, 768]），
    # 作为第2个make_last_layers()的输入，输出x的shape是[?, 26, 26, 256]，输出y2的shape是[?, 26, 26, num_anchors*(num_classes+5) ]。
    x = compose(
    ...
    # y3：输入是（x,y2)输出的x，shape是[?, 26, 26, 256]。
    # 先执行一次有128个滤波器（所以，输出是128个通道）的1x1卷积操作DarknetConv2D_BN_Leaky()；
    # 然后做一次上采样操作UpSampling2D()，将输入为26x26的结构，转换为52x52的结构。
    # 将x与darknet的第92层拼接Concatenate（拼接后张量的shape是[?, 52, 52, 384]），
    # 作为第3个make_last_layers()的输入，输出x忽略，输出y3的shape是[?, 52, 52, num_anchors*(num_classes+5) ]。
